...
public class FancyCoreUpdateCMD extends FancyLeafCommandBase {
        ...
        public FancyCoreUpdateCMD() {
                ...
                // requirePermission("fancycore.commands.fancycore.update"); // Handled manually
                // for localization
        ...
        protected void executeSync(@NotNull CommandContext ctx) {
                if (!checkPermission(ctx, "fancycore.commands.fancycore.update")) {
                        return;
                }
                ...
                try {
                        ...
                        ctx.sender().sendMessage(
                                        Message.raw("Successfully downloaded FancyCore version " + latestVersion.name()
                                                        + ". Please restart the server to apply the update."));
                        logger.info(
                                        "Successfully downloaded the latest version of FancyCore",
                                        StringProperty.of("version", latestVersion.name()),
                                        StringProperty.of("published_at",
                                                        TimeUtils.formatDate(latestVersion.publishedAt())),
                                        StringProperty.of("download_link", latestVersion.downloadURL()));
